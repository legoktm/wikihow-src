...
$count = 0;
do {
	$res = $dbw->select('qc', array('*'), array("qc_patrolled" => 1), "archiveQG.php", array("LIMIT" => 1000));
	$qcRows = array();
	$qcVoteRows = array();
	$qcIds = array();
	$moreRows = false;
	...
	if (sizeof($qcIds)) {
		$res1 = $dbw->select('qc_vote', array('*'), array(joinClause("qcv_qcid", $qcIds)), "archiveQG.php");
		while ($row = $dbw->fetchObject($res1)) {
