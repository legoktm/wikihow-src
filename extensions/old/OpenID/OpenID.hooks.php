...
class OpenIDHooks {
	...
	private static function getInfoTable( $user ) {
		...
		foreach( $urls as $url ) {
			$rows .= Xml::tags( 'tr', array(),
				...
				Xml::tags( 'td', array(), $sk->makeLinkObj( $delTitle, wfMsg( 'openid-urls-delete' ), wfArrayToCgi( array( 'url' => $url ) ) ) )
			) . "\n";
		}
	...
	public static function onSavePreferences( $prefs, $user, &$message, $old = null ) {
		foreach ( self::getToggles() as $oidPref ) {
