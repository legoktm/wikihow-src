...
class Minify {
     ...
     * better browser downloading parellelization, or remove it for
    ...
    protected static function _modCssCdn($content) {
        // A big hack so we don't have to re-define wfGetPad() -- define $wgServer
        global $wgServer;
        ...
        $isSSLsite = $serverName == 'ssl.wikihow.com';
        $isStageHost = $hostname == 'spare1.wikihow.com';
        $isDevHost = preg_match('@wikihow.com$@', $hostname) == 0;
        $isCachedCopy = @$_GET['c'] == 't';
        @include('../../env.php');

        if (!$isDevHost) {
            if (!$isSSLsite && !$isStageHost && !$isCachedCopy) {
                $content = self::_addCssCdn($content);
            } else {
                $content = self::_removeCssCdn($content);
            }
        }
        return $content;
    }
    ...
    /**
     * Remove padx.whstati.com URLs from CSS
     */
    protected static function _removeCssCdn($css)
    {
