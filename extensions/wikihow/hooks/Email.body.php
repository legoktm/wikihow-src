...
class EmailBounceHooks {
	...
	/**
	 * Returns email from strings. e.g. this will return "john@domain.com" from string below
	...
	private static function getDBBouncingEmails( $emailAddrs ) {
		if ( empty( $emailAddrs ) || !is_array ( $emailAddrs ) ) return null;
		...
		$res = $dbr->select( 'suppress_emails',
				array( 'email' ),
				array( 'email' => array_keys( $oemailEmailMap ) ),
				__METHOD__ );
	...
	/*
	private static function splitAddrs( &$to ) {
		$toRem = array();
		$toAdd = array();
		...
		foreach ( $to as $index => $addr ) {
			...
			if ( $pos !== false ) {
				$toRem[] = $index;
				$toAdd = array_merge( $toAdd, explode( ',', strval( $addr ) ) );
			}
			
		}
		
		if ( !empty($toRem) ) {
			foreach ( $toRem as $pos ) {
				unset( $to[$pos] );
			}
			
			foreach ( $toAdd as $emailAddr ) {
				$emailAddrNew = new MailAddress( trim( $emailAddr ) );
				$to[] = $emailAddrNew;
			}
		}
	...
	*/
	...
	public static function onFilterOutBouncingEmails( &$to ) {
		...
		if ( empty( $to ) || !is_array ( $to ) ) return false;
		...
		foreach ( $bEmailAddrs as $bemailAddr ) {
			$pos = array_search( $bemailAddr, $to );
			if ( $pos !== false ) unset( $to[$pos] );
		}
