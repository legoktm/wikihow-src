...
class NFDProcessor {
	...
	function getNextToPatrolHTML() {
		...
		$tmpl->set_vars(array(
			...
			'articleHtml' => WikihowArticleHTML::processArticleHTML($wgOut->parse($r->getText(), $t, $popts), array('no-ads'=> true, 'ns' => $t->getNamespace(), 'magic-word' => $magic)),
...
class NFDAdvanced extends QueryPage {
	...
	function getSQL() {
		return "SELECT nfd_page as title, nfd_fe_timestamp as value FROM nfd WHERE nfd_status = " . NFDGuardian::NFD_ADVANCED . " GROUP BY nfd_page";
	}
