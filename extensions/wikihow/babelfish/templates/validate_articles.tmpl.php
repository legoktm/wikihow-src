...
<h4><input type='checkbox' id='v_check_unassigned' /> Urls not assigned</h4>
<ul class='url_list'>
<?
	foreach ($urls['unassigned'] as $url) {
		...
		$ca = $url['a'];
		$url = $url['url'];
		echo "<li><input type='checkbox' value='$aid' langcode='$langCode' class='checked_article v_check_unassigned'/> $url</li>";
	}
?>
</ul>
<? } ?>
