...
class ImageTransfer {
	...
	public function reportSuccess($dryRun=true) {
		...
		print "Image transfer run successfully at " . $this->timeFinished;
 ...
 /**
	...
	*/
	public function addImages($dryRun = true) {
		...
		if(!$from) {
			$this->reportError("Unable to fetch article",  $dryRun);	
			return(false);
		}
	...
	private static function replaceStepsImages($stepsText, $stepTemplates, $token, $altImageTags) {
		...
		if((sizeof($steps) -1) != sizeof($stepTemplates)) {
			if(sizeof($steps)  == sizeof($stepTemplates)
		...
		foreach($sections as $section) { 
			...
			if(sizeof($steps) > 1) {
				for($i=1; $i < sizeof($steps); $i++) {
					...
					if($stepTemplates[$stepNum] != '' && !self::haveSameImages($stepTemplates[$stepNum], $steps[$i])) {
						$step = preg_replace(self::getImageRegex($altImageTags), '', $steps[$i]);
						$step = preg_replace('@<\s*br[\/]?\s*>|^#\*?@im','',$step);
						$step = preg_replace('@[\r\n]*@im','',$step);
						$step = preg_replace("@(#[*]?)\s*(.+)\s*$@","\\1 \\2",$step);
						$steps[$i] = str_replace($token, $step, $stepTemplates[$stepNum]);
		...
		if($addFinishedStep) {
			$txt .=  "\n" . str_replace($finishedToken, wfMsg('finished'), $stepTemplates[$stepNum]);
		}
