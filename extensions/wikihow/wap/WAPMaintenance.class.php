...
abstract class WAPMaintenance {
	...
	private function removeRedirects($startDate, $endDate) {
		...
		$sql = "SELECT de_page_id FROM daily_edits de LEFT JOIN page p ON de_page_id = page_id 
			WHERE (de_timestamp >= '$startDate' AND de_timestamp < '$endDate') AND de_edit_type = $type AND page_is_redirect = 1";
		$res = $dbr->query($sql, __METHOD__);
		$ids = array();
		foreach ($res as $row) {
			$ids[] = $row->de_page_id;
	...
	private function updateMovedArticles($startDate, $endDate) {
		...
		$dbr = wfGetDB(DB_SLAVE);
		$sql = "SELECT de_page_id FROM daily_edits 
			WHERE (de_timestamp >= '$startDate' AND de_timestamp < '$endDate') AND de_edit_type = $type";
		$res = $dbr->query($sql, __METHOD__);
		$ids = array();
		foreach ($res as $row) {
			$ids[] = $row->de_page_id;
	...
	private function removeBadTemplates($startDate, $endDate) {
		$dbr = wfGetDB(DB_SLAVE);
		$type = DailyEdits::EDIT_TYPE;
		$badTemplates = array('merge', 'nfd', 'copyvio', 'copyviobot', 'copyedit', 'cleanup', 'format', 'accuracy', 'speedy'); 
		...
		$sql = "SELECT de_page_id FROM daily_edits de LEFT JOIN templatelinks t ON de_page_id = tl_from 
			WHERE (de_timestamp >= '$startDate' AND de_timestamp < '$endDate') AND de_edit_type = $type AND tl_namespace = $ns AND 
	...
	*/
	...
	private function checkupMovedArticles() {
		$articleTable = $this->wapConfig->getArticleTableName();
		$type = DailyEdits::MOVE_TYPE;
		$sql = "SELECT ct_lang_code, ct_user_text, ct_user_id, ct_page_id, ct_page_title FROM $articleTable, page  WHERE ct_page_id = page_id and ct_page_title != page_title
				AND ct_page_id NOT IN (SELECT de_page_id FROM daily_edits WHERE de_timestamp >= '{$this->endDate}' AND de_edit_type = $type)";
		$this->genericCheckup($sql, self::MOVED_TYPE);
	}
